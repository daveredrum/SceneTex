...
from models.ControlNet.gradio_depth2image_repaint import init_model, process, process_with_prior
...
def get_controlnet_depth_inpainting(device):
    """
        adapt controlnet depth with stable diffusion inpainting via:
            https://github.com/haofanwang/ControlNet-for-Diffusers#controlnet--inpainting

    """
    print("=> initializing ControlNet Depth Inpainting...")

    pipe_inpaint = StableDiffusionInpaintPipeline.from_pretrained("runwayml/stable-diffusion-inpainting",torch_dtype=torch.float16).to(device)
    # NOTE ControlNet Depth2img checkpoint needs to be converted to diffusers format
    pipe_control = StableDiffusionControlNetInpaintPipeline.from_pretrained("models/diffusers/checkpoints/control_sd15_depth",torch_dtype=torch.float16).to(device)
...
@torch.no_grad()
def apply_controlnet_depth_inpainting(model, ddim_steps, strength, generator,
    init_image, generate_mask_image, depth_map_image,
    ...
    diffused_image = model(
        height=height,
        width=width,
        prompt="{}, {}".format(prompt, a_prompt), 
        negative_prompt=n_prompt,
        controlnet_hint=depth_map_image, 
        image=init_image,
    ...
    diffused_image = diffused_image.convert("RGB")
...
@torch.no_grad()
def apply_controlnet_depth_with_prior(model, ddim_sampler, init_image, prompt, 
    a_prompt, n_prompt, num_samples,  
    depth_map_np, prior_mask, unknown_mask,
    ddim_steps, guidance_scale, seed, eta, 
    prior_strength, ddim="uniform"):
    """
        Use Stable Diffusion 2 to generate image
    ...
    # image and mask_image should be PIL images.
    # The mask structure is white for inpainting and black for keeping as is
    diffused_image_np = process_with_prior(
        model, ddim_sampler, np.array(init_image), prompt, 
        a_prompt, n_prompt, num_samples,
        depth_map_np, prior_mask, unknown_mask,
        ddim_steps, guidance_scale, seed, eta, 
        prior_strength=prior_strength, ddim=ddim
    )[0]
